...
public class Main extends JFrame {
	...
	public Main() {
		...
		// 관리자 접속 메인창에서 아래는 각 탭의 테이블 내용을 더블 클릭했을 때
		// 새로운 창을 띄우기 위한 코드들임. 187라인까지
	...
	private void init() {
		...
		changeVisible(0);	// 하단 패널에 컴포넌트들 보이고 숨기기 조정
	...
	// Movie 테이블의 모든 데이터를 가져오되 keyword(null도 가능)로 검색해서 일치하는 결과들만
	// DefaultTableModel(스윙의 테이블)에 담아서 반환함
	public DefaultTableModel setMovieTable(String keyword) {
		// 테이블에 표시할 컬럼명이 Vector에 담겨 반환됨
		Vector<String> colName = util.getTableTitleColum("movie");

		// DefaultTableModel(Vector columnNames, int rowCount) 생성자 사용
		// DefaultTableModel 클래스를 바로 오버라이딩한 익명 클래스임
		...
		// keyword가 null인 경우는 Movie 테이블의 모든 row를 가져옴
		if (keyword == null) {
		...
		// Vector에 담겨 테이블에서 가져온 내용들을 하나씩 불러와 스윙 테이블에 내용을 채움
		for (Movies s : movies) {
			Vector<Object> row = new Vector<>();
	...
	public DefaultTableModel setScreenTable(String keyword) {
		// Screen 테이블에 사용할 컬럼명들을 Vector로 가져옴
		Vector<String> colName = util.getTableTitleColum("screen");
		
		// DefaultTableModel 클래스를 오버라이딩한 익명 클래스임
		...
		// Screen 테이블로부터 가져온 정보를 차례로  DefaultTableModel 객체에 채움
	...
	public DefaultTableModel setPlaceTable(String keyword) {
		// place 테이블에 사용할 필드명을 가져옴
		Vector<String> colName = util.getTableTitleColum("place");

		// DefaultTableModel 클래스를 오버라이딩한 익명클래스 객체를 생성함
